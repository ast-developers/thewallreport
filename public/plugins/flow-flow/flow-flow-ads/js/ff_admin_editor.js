...
(function($) {
  ...
  $.tablesort.prototype = {
    ...
    sort: function(th, direction) {
      ...
      setTimeout(function() {
        ...
        {
          numVal = parseInt(unsortedValues[i]);
          sortedMap.push({
            index: i,
            cell: cells[i],
            row: rows[i],
            value: isNaN(numVal) ? unsortedValues[i] : numVal
          });
        }
  ...
  $.tablesort.defaults = {
    ...
    asc: 'ff-sorted ff-ascending',
    desc: 'ff-sorted ff-descending'
  };
...
(function() {
  ...
  _EditorApp = (function(_super) {
    ...
    _EditorApp.prototype.busy = function(busy) {
      return this._ignition ? this._ignition.busy(busy) : false;
    };
    ...
    _EditorApp.prototype.init = function(queryOrDOMElements, namingProp) {
      ...
      // console.log('ContentEdit.Root init reset');
    ...
    _EditorApp.prototype.start = function() {
      ...
      if (this._toolbox) this._toolbox.show();
      if (this._inspector) this._inspector.show();
    ...
    _EditorApp.prototype.stop = function() {
      ...
      if (this.history) this.history.stopWatching();
      this.history = null;
      if (this._toolbox) this._toolbox.hide();
  ...
  ContentTools.Tools.Preformatted = (function(_super) {
    ...
    Preformatted.label = 'Button';

    Preformatted.icon = 'preformatted';

    Preformatted.tagName = 'p';

    Preformatted.className = 'ff-ad-cta';
    ...
    Preformatted.canApply = function(element, selection) {
      return true;
    };

    Preformatted.apply = function(element, selection, callback) {
      var button, index, node, _ref;

      var content = 'Call to Action';
      button = new ContentEdit.Text(this.tagName, {'data-ct-type': 'button'}, content);
      _ref = this._insertAt(element), node = _ref[0], index = _ref[1];
      node.parent().attach(button, index);
      button.addCSSClass(this.className);
      button.focus();
      return callback(true);
    };
    return Preformatted;
  ...
  ContentTools.Tools.Image = (function(_super) {
    ...
    Image.apply = function(element, selection, callback) {
      ...
      window.file_frame.on( 'select', function() {
        ...
        imageAttrs.src = attachment.url;
        imageAttrs.height = attachment.height;
        imageAttrs.width = attachment.width;
        image = new ContentEdit.Image(imageAttrs);
        _ref = self._insertAt(element), node = _ref[0], index = _ref[1];
        node.parent().attach(image, index);
        image.focus();

        // CUSTOMIZATION
        var offset = jQuery(image._domElement).offset()
        image.resize(["bottom", "right"], offset.left, offset.top);
