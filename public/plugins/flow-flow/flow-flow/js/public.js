        ...
        var u = function(t) {
            ...
            function f(t, e, n, s, r, a) {
                ...
                for (a || (a = 0), r && "randomCompare" == r.order && (t = C(t)), o = 0; E > o; o++) {
                    if (f = t[o], l = o + 1, v = O = g = N = z = "", _ = {}, k = r.isOverlay && (!!f.img || -1 !== f.text.indexOf("<img")), "ad" !== f.type) {
                        m = void 0 !== f.source ? f.source : f.permalink, s && (I = "new" == f.status ? " ff-moderation-new-post" : "", S = "approved" == f.status ? "checked" : "", z = f.mod ? '<div class="ff-moderation-wrapper ' + ("approved" == f.status ? "ff-approved" : "") + I + '"><span>Approve</span> <label for="ff-mod-' + ($ + a) + '"><input id="ff-mod-' + ($ + a) + '" type="checkbox" class="ff-switcher" value="yes" ' + S + '/><div><span></span></div></label></div>':'', d = FlowFlowOpts.isAdmin ? '<div class="ff-moderation-wrapper switch-response"></div><div class="ff-moderation-wrapper ff-moderation-featured-wrapper' + I + '"><span>Featured</span> <label for="ff-featured-' + ($ + a) + '"><input id="ff-featured-' + ($ + a) + '" type="checkbox" class="ff-switcher" value="yes"  ' + ("1" == f.featured ? "checked" : "") + '/><div><span></span></div></label></div><div class="ff-moderation-wrapper ff-moderation-is-active-wrapper' + I + '"><span>Show/Hide</span> <label for="ff-is-active-' + ($ + a) + '"><input id="ff-is-active-' + ($ + a) + '" type="checkbox" class="ff-switcher" value="yes" ' + ("1" == f.is_active ? "checked" : "") + "/><div><span></span></div></label></div>":''), f.additional && ("twitter" === f.type && (v += '<a href="https://twitter.com/intent/tweet?in_reply_to=' + f.id + '" class="ff-comments"> <i class="ff-icon-reply"></i></a>'), (b = parseInt(f.additional.views)) > -1 && (v += '<a href="' + f.permalink + '" class="ff-views"><i class="ff-icon-view"></i> <span>' + (0 > b ? "" : i(b, 2)) + "</span></a>"), (y = parseInt(f.additional.likes)) > -1 && "twitter" !== f.type && (v += '<a href="' + f.permalink + '" class="ff-likes"><i class="ff-icon-like"></i> <span>' + (0 > y ? "" : i(y, 2)) + "</span></a>"), (x = parseInt(f.additional.shares)) > -1 && (v += '<a href="' + ("twitter" === f.type ? "https://twitter.com/intent/retweet?tweet_id=" + f.id : f.permalink) + '" class="ff-shares"><i class="ff-icon-shares"></i> <span>' + (0 > x ? "" : i(x, 2)) + "</span></a>"), y > -1 && "twitter" === f.type && (v += '<a href="https://twitter.com/intent/favorite?tweet_id=' + f.id + '" class="ff-likes"><i class="ff-icon-like"></i> <span>' + (0 > y ? "" : i(y, 2)) + "</span></a>"), w = parseInt(f.additional.comments), "twitter" !== f.type && w > -1 && (v += '<a href="' + f.permalink + '" class="ff-comments"><i class="ff-icon-comment"></i> <span>' + (w > -1 ? i(w, 2) : "") + "</span></a><a href='" + window.base_url + "feed/" + f.id + "/" + f.feed_id + "' class='external-link'><i class='fa fa-external-link-square'></i></a>")), n && (O += '<div class="ff-share-wrapper"><i class="ff-icon-share"></i><div class="ff-share-popup"><a href="http://www.facebook.com/sharer.php?u=' + (u = encodeURIComponent(f.permalink)) + '" class="ff-fb-share"><span>Facebook</span></a><a href="https://twitter.com/share?' + (f.header ? "text=" + encodeURIComponent(f.header) + "&" : "") + "url=" + u + '" class="ff-tw-share"><span>Twitter</span></a><a href="https://plus.google.com/share?url=' + u + '" class="ff-gp-share"><span>Google+</span></a><a href="https://www.pinterest.com/pin/create/button/?url=' + u + (f.media ? "&media=" + encodeURIComponent(f.media.url) : "") + '" class="ff-pin-share"><span>Pinterest</span></a></div></div>'), N = f.media ? ' data-media="' + f.media.width + ";" + f.media.height + ";" + ("yep" === p.forceHTTPS ? f.media.url.replace("http:", "https:") : f.media.url) + ";" + f.media.type + (f.img ? ";" + f.img.width + ";" + f.img.height : "") + '"' : "", _.image = f.img ? '<span class="ff-img-holder ff-img-loading" ' + N + '><img class="ff-initial-image" src="' + ("yep" === p.forceHTTPS ? f.img.url.replace("http:", "https:") : f.img.url) + '" data-size="' + f.img.width + ";" + f.img.height + '" /></span>' : "", _.header = f.header ? '<h4><a rel="nofollow" href="' + m + '">' + f.header + "</a></h4>" : "", _.text = '<div class="ff-content">' + f.text + "</div>", _.meta = '<div class="ff-item-meta"><span class="ff-userpic" style="background:url(' + ("yep" === p.forceHTTPS ? f.userpic.replace("http:", "https:") : f.userpic) + ')"><i class="ff-icon"><i class="ff-icon-inner"></i></i></span><h6><a rel="nofollow" href="' + f.userlink + '" class="ff-name ' + (f.userlink ? "" : " ff-no-link") + '">' + f.screenname + "</a></h6>" + (f.nickname ? '<a rel="nofollow" href="' + f.userlink + '" class="ff-nickname' + (f.userlink ? "" : " ff-no-link") + '">' + f.nickname + "</a>" : "") + '<a rel="nofollow" href="' + f.permalink + '" class="ff-timestamp">' + T(f.system_timestamp, f.timestamp) + "</a></div>", _.labelIcon = F ? "" : '<h6 class="ff-label-wrapper"><i class="ff-icon"><i class="ff-icon-inner"><span class="ff-label-text">' + f.type + "</span></i></i></h6>";
                        ...
                        M += '<article class="ff-item' + (f.media && "image" != f.media.type ? " ff-video-preview" : "") + " ff-" + f.type + ("meta" === r.template[r.isOverlay ? 1 : 0] || !f.img && "meta" === r.template[1] ? " ff-meta-first" : "") + (f.header ? " ff-has-header" : "") + (f.img ? " ff-image" : " ff-no-image") + (k ? " ff-has-overlay" : "") + '" id="ff-uid-' + $ + '" post-id="' + f.id + '" data-type="' + f.type + '" data-index="' + (l + a) + '"' + N + ' data-timestamp="' + f.system_timestamp + '">' + (s && f.mod ? z : "") + (s && FlowFlowOpts.isAdmin ? d : "") + '<div class="picture-item__inner"><div class="ff-item-cont">' + g + "</div>" + (k ? "" : '<h6 class="ff-item-bar">' + v + O + "</h6>"), d && (M += '<a class="ff-mob-link" href="' + f.permalink + '"></a>'), M += "</div></article>"
                    } else h = "yep" === f.label ? 'data-label="' + f.labelTxt + ";" + f.labelCol + '"' : "", c = 'style="' + (f.textCol ? "color:" + f.textCol + ";" : "") + ("js" === f.adtype ? "height:" + f.height + "px" : "") + '"', M += '<div class="ff-item ff-' + f.type + (f.permalink ? " ff-ad-link" : "") + '" id="ff-uid-' + $ + '" post-id="' + f.id + '" data-type="' + f.type + '" data-adtype="' + f.adtype + '" data-index="' + l + '" ' + h + '><div class="picture-item__inner" style="' + (f.cardBG ? "background-color:" + f.cardBG + ";" : "") + '"><div class="ff-item-cont"><div class="ff-content" ' + c + ">" + f.text.replace(/document\.write\((.+?)\)/i, function(t, e) {
                            return "jQuery(" + e + ').appendTo(jQuery("#ff-uid-' + $ + ' .ff-content"))'
                        }) + "</div>", f.permalink && (M += '<a class="ff-link" href="' + f.permalink + '"></a>'), M += "</div></div></div>";
                    $++
                }
                return M
            }
