            ...
            function f(t, e, n, s, r, a) {
                ...
                for (a || (a = 0), r && "randomCompare" == r.order && (t = C(t)), o = 0; E > o; o++) {
                    if (f = t[o], l = o + 1, v = O = g = N = z = "", _ = {}, k = r.isOverlay && (!!f.img || -1 !== f.text.indexOf("<img")), "ad" !== f.type) {
                        ...
                        M += '<article class="ff-item' + (f.media && "image" != f.media.type ? " ff-video-preview" : "") + " ff-" + f.type + ("meta" === r.template[r.isOverlay ? 1 : 0] || !f.img && "meta" === r.template[1] ? " ff-meta-first" : "") + (f.header ? " ff-has-header" : "") + (f.img ? " ff-image" : " ff-no-image") + (k ? " ff-has-overlay" : "") + '" id="ff-uid-' + $ + '" post-id="' + f.id + '" data-type="' + f.type + '" data-index="' + (l + a) + '"' + N + ' data-timestamp="' + f.system_timestamp + '">' + (s && f.mod ? z : "") + (s && FlowFlowOpts.isAdmin ? d : "") + '<div class="picture-item__inner">'+((f.featured == "1" &&FlowFlowOpts.isAdmin!=1) ? "<div class='featured-badge'></div></span>" : "")+'<div class="ff-item-cont">' + g + "</div>" + (k ? "" : '<h6 class="ff-item-bar">' + v + O + "</h6>"), d && (M += '<a class="ff-mob-link" href="' + f.permalink + '"></a>'), M += "</div></article>"
